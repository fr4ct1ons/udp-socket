...
int main()
{
	...
	if (bind(skt2, (sockaddr*)&client2Info, sizeof(client2Info)) == SOCKET_ERROR)
	{
		std::cout << "ERROR BINDING SOCKET - error: " << WSAGetLastError() << std::endl;
		return 2;
	}
	...
	while (true)
	{
		...
		if (recvfrom(skt2, buffer2, 1024, 0, (sockaddr*)&client2, &client2Length) == SOCKET_ERROR)
		{
			std::cout << "ERROR RECEIVING MESSAGE FROM CLIENT - error: " << WSAGetLastError() << std::endl;
			continue;
		}
		...
		//String buffer
		buffer = buffer2;
		if (sendto(skt, buffer.c_str(), buffer.size() + 1, 0, (sockaddr*)&client1Info, sizeof(client1Info)) == SOCKET_ERROR)
		{
			std::cout << "ERROR SENDING MESSAGE - error: " << WSAGetLastError() << std::endl;
		}
		else
